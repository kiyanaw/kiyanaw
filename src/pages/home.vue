<template>
  <f7-page>
    ...
    <f7-navbar>
      <f7-nav-left>
        <f7-link class="panel-open" open-panel="left" icon="fas fa-bars" />
      </f7-nav-left>
      ...
      <f7-nav-right>
        <f7-link ref="searchLink" class="searchbar-enable" icon-md="material:search" />
      </f7-nav-right>
      <f7-searchbar
        ref="searchbar"
    ...
    <f7-block v-if="!searching && !searchText">
      ...
      <f7-row>
        <f7-col>
          <f7-button raised icon-md="material:search" class="searchbar-enable">
            Search
          </f7-button>
        </f7-col>
        <f7-col>
          <f7-button raised icon-md="material:list" href="/browse/">
            Browse
          </f7-button>
        </f7-col>
      </f7-row>
    ...
    <div v-if="searching && !searchText">
      ...
      <f7-list class="nomargin">
        <f7-list-item title="visits" @click="$refs.searchbar.search('visits')" />
        <f7-list-item title="fontanelle" @click="$refs.searchbar.search('fontanelle')" />
        <f7-list-item title="itâhkamikisiw" @click="$refs.searchbar.search('itâhkamikisiw')" />
      </f7-list>
    </div>
    ...
    <f7-fab v-if="user" slot="fixed" position="right-bottom" color="blue">
      ...
      <f7-fab-buttons position="top" color="green">
        <f7-fab-button fab-close href="/new-enquiry/">
          <f7-icon md="material:create" size="15" />
        </f7-fab-button>
      </f7-fab-buttons>
    </f7-fab>
